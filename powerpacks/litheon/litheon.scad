...
include <../../models/model_litheon_18v.scad>
...
module housing_3pin()
{
    tag("") cuboid([400,2080,559],anchor=BOT+LEFT,rounding=100,edges=[RIGHT+FRONT,RIGHT+BACK]) {
        tag("remove") down(1) attach(LEFT+BOT,norot=1)
            cuboid([250,1880,459+1],anchor=LEFT+BOT)
...
module clip()
{
    ...
    diff() {
        cuboid([3000,2650,560], rounding=200,edges="Z")
        {
            tag("remove")
            {
...
//          attach(TOP+RIGHT,norot=1) back(2085/2) cuboid([2780,86,115], anchor=BACK+RIGHT+BOT)
//          attach(TOP+BACK,norot=1) cuboid([2780,160,100], anchor=BOT+BACK);
...
//          attach(TOP+RIGHT,norot=1) fwd(2085/2) cuboid([2780,86,115], anchor=FRONT+RIGHT+BOT)
//          attach(TOP+FRONT,norot=1) cuboid([2780,160,100], anchor=BOT+FRONT);
