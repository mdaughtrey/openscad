...
//    radius = radius - (radius/6); 
    rotate_extrude($fn=96) 
    {
        ...
        hull()
        {
            ...
            //intersection() { circle(r=corner); translate([0, -corner, 0]) square(corner); }

            translate([0, height, 0]) square(corner); // inside upper
            translate([radius-corner, height, 0])    // outside upper
            intersection() { circle(r=corner); square(corner); }
        }
    }
}
...
module jointB()
{
    difference() {
        // Inner joint
//        translate([0, 0, -25]) cap(LayerUnit, 400, 100);
    ...
    difference() {
        rotate_extrude($fn=96) 
        {
            hull()
            {
                translate([0, 0, 0]) square(corner, 0); // inside lower
                //translate([radius-corner, 0, 0]) square(corner, 0); // outside lower
                translate([radius-corner, corner, 0]) // square(corner, 0); // outside lower
                intersection() { circle(r=corner); translate([0, -corner, 0]) square(corner); }
        ...
        union() {
            translate([StrutWidth/2, -JointR*2, -100]) cube([JointR*2, JointR*4, LayerUnit*2]);
            translate([-StrutWidth/2-JointR*2, -JointR*2, -100]) cube([JointR*2, JointR*4, LayerUnit*2]);
            translate([JointR, 0, -100]) rotate([0, 0, 180]) cube([JointR*2, JointR*3, LayerUnit*2]);
            translate([0, 0, -1]) cylinder(LayerUnit+2, r=JointR+eO, $fn=96);
        }
    }
}
...
module footStrutB(rodw, rodl)
{
    ...
    translate([0, 0, 0]) rotate([0, 0, -90]) jointExt(StrutWidth, LayerUnit, 0);
    // lower extension
    translate([0, 0, -LayerUnit])
    //linear_extrude(SpacedLayer)
    translate([0, 0, 0]) rotate([0, 0, -90]) jointExt(StrutWidth, LayerUnit, 30);
